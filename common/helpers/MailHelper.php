...
class MailHelper
{
     ...
     * @param $errorContent
     */
    public static function mailer($errorTitle, $errorContent)
    {
        $email_data = Yii::$app->params['adminEmail'];
        if ($email_data) {
            foreach ($email_data as $email) {
                Yii::$app->services->mailer->businessSend($email, $errorTitle, $errorContent);
            }
        }
    }
...
//            return true;
    ...
    protected function getCommand()
    {
        ...
        $command[] = array("MAIL FROM:<" . $this->_from[1] . ">\r\n", 250);
        $separator = "----=_Part_" . md5($this->_from[1] . time()) . uniqid(); //分隔符
        //设置发件人
        $header = "FROM: ". $this->_from[0] ."<" . $this->_from[1] . ">\r\n";
        //设置收件人
...
//             echo 'Send command:' . $command . ',expected code:' . $code . '<br />';
        ...
        try {
            if (socket_write($this->_socket, $command, strlen($command))) {
                ...
                //读取服务器返回
                $data = trim(socket_read($this->_socket, 2048));
...
// 				$mime = mime_content_type($this->_attachment);
            $mime = system(trim('file -bi ' . escapeshellarg($this->_attachment)));
            if (!preg_match("/gif|jpg|png|jpeg|txt|xls/", $mime)) {
