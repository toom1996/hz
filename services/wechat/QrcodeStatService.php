...
class QrcodeStatService extends Service
{
    ...
    public function scan($message)
    {
        ...
        if ($qrCode = Yii::$app->services->wechatQrcode->findByWhere($where)) {
            ...
            if ($qrCode['keyword'] == QrcodeStat::$typeExplain[QrcodeStat::TYPE_BIND]) {
                $member_id = HashidsHelper::decodeHex($message['EventKey']);
                return Auth::bind(Auth::CLIENT_WECHAT, $message['FromUserName'], $member_id);
            }
            return $qrCode['keyword'];
        }
