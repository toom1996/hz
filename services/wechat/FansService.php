...
class FansService extends Service
{
    ...
    public function follow($openid)
    {
        ...
        $fans->role_id = UserEnum::CUSTOMER_ROLE;
        //不确定微信字段
        $fans->name = $user['nickname'];
        $fans->country = $user['country'];
        $fans->province = $user['province'];
        $fans->city = $user['city'];
        $fans->sex = $user['sex'];
        $fans->login_timestamp = time();
        $fans->login_type = UserEnum::SCAN_CODE;
        $fans->name = $user['province'];


        $fans->openid = $openid;
        $fans->head_portrait = $user['headimgurl'];
        $fans->subscribe = WechatEnum::FOLLOW_ON;
        $fans->create_datetime = date('Y-m-d H:i:s',time());
        $fans->save();
        return $fans;
//        Yii::$app->services->wechatFansStat->upFollowNum();
    ...
    {
        if (empty($openid) || empty(($model = User::find()->where(['openid' => $openid])->one()))) {
            return new User();
        }
