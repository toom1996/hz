...
class SmsService extends Service
{
    ...
    public function realSend($mobile, $code, $usage, $member_id = 0)
    {
        ...
        try {
            // 校验发送是否频繁
            if (($smsLog = $this->findByMobile($mobile, $usage)) && $smsLog['created_at'] + 300 > time()) {
                throw new NotFoundHttpException('验证码暂未失效哦~');
            }
            ...
            return ResultDataHelper::api(200, '发送成功');
     ...
     */
    public function findByMobile($mobile, $usage)
    {
        return SmsLog::find()
            ->where(['mobile' => $mobile, 'usage' => $usage, 'used' => StatusEnum::ENABLED])
            ->orderBy('id desc')
            ->asArray()
            ->one();
    }
